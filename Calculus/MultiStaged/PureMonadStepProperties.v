...
Module Type PureMonadStepProperties (R:Replacement) (S:ReplacementCalculus R)
  ...
  Parameter astep_assign_loc :
    ...
    (phi v bs)) (trans_mem (S.Memory.set l v M) bs, 
    ret (phi v bs)).
  ...
  Parameter astep_erun :
    ...
    S.svalue 1 e ->
    astep (M, cast_erun (cast_ebox (trans_expr e bs))) 
    (M, trans_expr e bs).

  Parameter astep_elift :
    forall (M:Memory.t) (e:S.expr) (bs:list nat),
    S.svalue 0 e ->
    astep (M, cast_elift (phi e bs)) 
    (M, ret (cast_ebox (ret (phi e bs)))).

End PureMonadStepProperties.
