...
Module ContextImpl (R:Replacement) (S:ReplacementCalculus R) 
    (T:StagedCalculus) (M:Monad R S T) : Context R S T M.
  Include Context R S T M.
End ContextImpl.
...
Module Type Translation (R:Replacement) (S:ReplacementCalculus R)
  ...
  Module Context := ContextImpl R S T M.
  Import S.CRaw.
  ...
  Fixpoint trans (e:S.expr) (bs:list nat) (dg:M.dg_t) (dgs:list M.dg_t) : T.expr * Context.t_stack :=
    ...
    | ERef e => 
        let (e,cs) := trans e bs (M.dg_eref dg) dgs in
        (M.bind dg e (fun v => M.cast_eref dg v), cs)
    | EDeref e => 
        let (e,cs) := trans e bs (M.dg_ederef dg) dgs in
        (M.bind dg e (fun v => M.cast_ederef dg v), cs)
    | EAssign e1 e2 => 
	let bs2 := map_iter_booker e2 bs 0 in
        let (e1', cs1) := trans e1 bs2 (M.dg_eassign_l dg) dgs in
        let (e2', cs2) := trans e2 bs (M.dg_eassign_r dg) dgs in
          (if svalueb 0 e1 then
	   M.bind dg e2' (fun v2 => M.cast_eassign dg 
             (phi e1 bs2 (M.dg_eassign_l dg) dgs) v2)
	   else M.bind dg e1' (fun v1 => M.bind dg e2'
          (fun v2 => M.cast_eassign dg v1 v2)), 
          Context.merge cs1 cs2)
    | EBox e => 
        let (e, cs) := trans e (0 :: bs) (M.dg_ebox dg) (dg::dgs) in
        match cs with
    ...
    | EUnbox e =>
        ...
        let (dg', dgs') := List2.hd_cons dgs M.dg_empty in
        let (e', cs) := trans e bs dg' dgs' in
           (M.cast_eunbox dg (M.cast_evar dg 
	   (M.cast_var (hole_var b))), ((e', b) :: nil) :: cs)
    | ERun e =>
        let (e,cs) := trans e bs (M.dg_erun dg) dgs in
        (M.bind dg e (fun v => M.cast_erun dg v), cs)
    | ELift e =>
        let (e,cs) := trans e bs (M.dg_elift dg) dgs in
        (M.bind dg e (fun v => M.cast_elift dg v), cs)
    end

  with phi (e:S.expr) (bs:list nat) (dg:M.dg_t) (dgs:list M.dg_t) : T.expr :=
    match e with
    | EConst i => M.cast_econst dg i
    | EVar y => M.cast_evar dg (M.cast_var (source_var y))
    | EAbs y e => 
        let (e, _) := trans e bs (M.dg_eabs dg y) dgs in
        M.cast_eabs dg (M.cast_var (source_var y)) e
    | EFix f y e => 
        let (e, _) := trans e bs (M.dg_efix dg f y) dgs in
        M.cast_efix dg
            (M.cast_var (source_var f)) 
            (M.cast_var (source_var y)) e
    | ELoc l => M.cast_eloc dg l
    | EBox e => 
        let (e, _) := trans e (0 :: bs) (M.dg_ebox dg) (dg::dgs) in
        M.cast_ebox dg e
    | _ => M.cast_econst dg 0
    end.

  Definition trans_expr (e:S.expr) (bs:list nat) (dg:M.dg_t) (dgs:list M.dg_t) : T.expr :=
    let (e, _) := trans e bs dg dgs in e.

  Fixpoint trans_mem (m:S.Memory.t) (bs:list nat) (dg:M.dg_t) (dgs:list M.dg_t) : T.Memory.t :=
    match m with
    | nil => nil
  ...
  Inductive admin : relation T.expr :=
    ...
    | Admin_abs : forall (x:T.var) (e1 e2:T.expr) (dg:M.dg_t),
        admin e1 e2 -> admin (M.cast_eabs dg x e1) (M.cast_eabs dg x e2)
    | Admin_fix : forall (f x:T.var) (e1 e2:T.expr) (dg:M.dg_t),
        admin e1 e2 -> admin (M.cast_efix dg f x e1) (M.cast_efix dg f x e2)
    | Admin_app : forall (e1 e2 e3 e4:T.expr) (dg:M.dg_t),
        admin e1 e3 -> admin e2 e4 -> 
        admin (M.cast_eapp dg e1 e2) (M.cast_eapp dg e3 e4)
    | Admin_ref : forall (e1 e2:T.expr) (dg:M.dg_t),
        admin e1 e2 -> admin (M.cast_eref dg e1) (M.cast_eref dg e2)
    | Admin_deref : forall (e1 e2:T.expr) (dg:M.dg_t),
        admin e1 e2 -> admin (M.cast_ederef dg e1) (M.cast_ederef dg e2)
    | Admin_assign : forall (e1 e2 e3 e4:T.expr) (dg:M.dg_t),
        admin e1 e3 -> admin e2 e4 -> 
        admin (M.cast_eassign dg e1 e2) (M.cast_eassign dg e3 e4)
    | Admin_box : forall (e1 e2:T.expr) (dg:M.dg_t),
        admin e1 e2 -> admin (M.cast_ebox dg e1) (M.cast_ebox dg e2)
    | Admin_unbox : forall (e1 e2:T.expr) (dg:M.dg_t),
        admin e1 e2 -> admin (M.cast_eunbox dg e1) (M.cast_eunbox dg e2)
    | Admin_run : forall (e1 e2:T.expr) (dg:M.dg_t),
        admin e1 e2 -> admin (M.cast_erun dg e1) (M.cast_erun dg e2)
    | Admin_lift : forall (e1 e2:T.expr) (dg:M.dg_t),
        admin e1 e2 -> admin (M.cast_elift dg e1) (M.cast_elift dg e2)
    | Admin_ret : forall (e1 e2:T.expr) (dg:M.dg_t),
        admin e1 e2 -> admin (M.ret dg e1) (M.ret dg e2)
    | Admin_bind : forall (e1 e2:T.expr) (f1 f2:T.expr -> T.expr) (dg:M.dg_t),
        admin e1 e2 -> (forall e3:T.expr, admin (f1 e3) (f2 e3)) ->
        admin (M.bind dg e1 f1) (M.bind dg e2 f2)
   | Admin_unbox_box : forall (e:expr) (bs:list nat) (dg:M.dg_t) (dgs:list M.dg_t),
        svalue 1 e ->
        admin (M.cast_eunbox dg (M.cast_ebox dg 
          (trans_expr e bs dg dgs))) 
          (trans_expr e bs dg dgs)
   | Admin_bind_app_phi : forall (v:expr) (e:T.expr) (bs:list nat) (dg:M.dg_t) (dgs:list M.dg_t), 
       svalue 0 v ->
       let f := fun v0 => M.bind dg e (fun v1 => M.cast_eapp dg v0 v1) in
       admin (M.bind dg (M.ret (M.dg_eapp_l dg) (phi v bs (M.dg_eapp_l dg) dgs)) f) (f (phi v bs (M.dg_eapp_l dg) dgs))
  | Admin_bind_assign_phi : forall (v:expr) (e:T.expr) (bs:list nat) (dg:M.dg_t) (dgs:list M.dg_t), 
       svalue 0 v ->
       let f := fun v0 => M.bind dg e (fun v1 => M.cast_eassign dg v0 v1) in
       admin (M.bind dg (M.ret (M.dg_eassign_l dg) (phi v bs (M.dg_eassign_l dg) dgs)) f) (f (phi v bs (M.dg_eassign_l dg) dgs)).
